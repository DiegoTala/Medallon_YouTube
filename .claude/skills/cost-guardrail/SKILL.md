...
Producir una estimación **estática** (basada en el pricing público de GCP y los supuestos de volumen del PRD, no en la Billing API en tiempo real) de cuánto agrega o resta un cambio propuesto, y compararla contra el techo mensual de $15.00 USD. Esta estimación es el insumo obligatorio que [[approval-gate]] exige mostrar antes de cualquier `apply`/`destroy`.

## Línea base del PRD (PRD §6)
...
| **Total estimado** | **~$1.40 – $1.80 / $15.00 techo** |
...
Costo total estimado tras el cambio: $Z.ZZ / $15.00 USD (techo PRD)
Margen restante: $(15.00 - Z.ZZ) USD (~W% del techo)
```

Si `Z.ZZ` se acerca a $15.00 (ej. >70% del techo) o lo supera, se marca explícitamente como **ALERTA** en el mensaje a [[approval-gate]] — no basta con mostrar el número, hay que señalarlo.
...
- **Siempre relativo al techo de $15 USD/mes**, nunca un número aislado sin contexto de cuánto margen queda.
